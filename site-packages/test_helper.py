...
# A permanent logFolder EV makes one log in the location defined in the variable.
# Otherwise the logFolder EV is set dynamically to the first script caller.
...
file = os.path.join(log_folder,"error_log")
            ...
            while True:
                try:
                    ...
                    if image_updating:
                        ...
                        SikuliImage.clearCache(0)
                        continue
                    else:
                        screenshot = area.getLastScreenImageFile()
                        #screenshot = sikuli.capture(area)
                        _write("---",scriptname, output)
...
def generateHTML():
    frames = inspect.getouterframes(inspect.currentframe())
    if not  frames[len(frames)-1][3] == "<module>":
